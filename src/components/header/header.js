...
function getPageTitle(menuItem, currentPath) {
  ...
  return currentItem ? `<h2 class="page-title__name">${currentItem.MENU_LIST}</h2>` : '';
}

function getUserName(userName) {
  return userName.map(item => `<span class="page-title__user">안녕하세요, ${item.NAME}님</span>`).join('');
}

export async function header() {
  const currentStorage = window.sessionStorage;
  const currentPath = window.location.pathname;
  const [menuData, userData] = await Promise.all([fetchData(`/api/menu/${currentStorage.getItem.role}`), fetchData(`/api/user/${currentStorage.num}`)]);
